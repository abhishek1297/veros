...
The root-level `summary.md` explains what each plot shows, how to interpret the
metrics, and how to locate and open the XProf traces. It is documentation and
is not generated by Snakemake.
...
For example, the traces for a two-node, four-GPU-per-node strong-scaling case
are rooted at `results/scaling/strong/nodes2-gpus4/jax-traces/`. Each
`repeatN-rankN/` directory contains the XProf files for one MPI rank, under the
JAX-generated `plugins/profile/` hierarchy.
